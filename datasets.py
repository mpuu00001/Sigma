...
class Base_Dataset(Dataset.Dataset):
    def collate_fn(self, batch):
        tgt_batch,src_length_batch,name_batch,pose_tmp,gloss_batch,label_key_batch,action_labels_batch,action_boundaries_batch = [],[],[],[],[],[],[],[]
        ...
        for name_sample, pose_sample, text, gloss, _, label_key, _ in batch:
            ...
            # action_labels_batch.append(torch.tensor(action_info['action_labels'], dtype=torch.long))
            # action_boundaries_batch.append(torch.tensor(action_info['boundaries'], dtype=torch.long))

        src_input = {}

        # if self.args.dataset == 'OpenASL':
        #     keys = pose_tmp[0][0].keys()
        #     tmp = []
        #     for tup in pose_tmp:
        #         tmp.append(tup[0])
        #     pose_tmp = tmp
        ...
        # padded_labels = pad_sequence(action_labels_batch, batch_first=True, padding_value=0)
        # padded_bounds = pad_sequence(action_boundaries_batch, batch_first=True, padding_value=0)
        # src_input['action_labels']     = padded_labels
        # src_input['action_boundaries'] = padded_bounds
        # src_input['action_len']        = [len(x) for x in action_labels_batch]
...
class S2T_Dataset(Base_Dataset):
    def __init__(self, path, args, phase):
        ...
        if self.args.dataset in ["CSL_Daily", "OpenASL", "NationalCSL-DP"]:
            self.pose_dir = pose_dirs[args.dataset]
            self.rgb_dir = rgb_dirs[args.dataset]
        elif "WLASL" in self.args.dataset or "MSASL" in self.args.dataset or self.args.dataset == 'How2Sign':
            dataset_name = re.sub(r'\d+', '', args.dataset) if self.args.dataset != 'How2Sign' else args.dataset
            self.pose_dir = os.path.join(pose_dirs[dataset_name], phase)
            self.rgb_dir = os.path.join(rgb_dirs[dataset_name], phase)
        else:
            raise NotImplementedError

        self.list = list(self.raw_data.keys())

        if self.args.dataset == "NationalCSL-DP":
            filtered_list = []
            for k in self.list:
                sample = self.raw_data[k]
                pose_name = sample.get("front_pose_path", "")
                pose_path = os.path.join(self.pose_dir, pose_name)
                if os.path.exists(pose_path):
                    filtered_list.append(k)
                else:
                    print(f"[NationalCSL-DP] skip sample {k} because pose not found: {pose_path}")
    ...
    def __getitem__(self, index):
        ...
        if (self.args.dataset == "How2Sign" or self.args.dataset == "OpenASL") and not self.args.rgb_support:
            pose_sample, _ = self.load_pose(sample['pose'])
            support_rgb_dict = None
        elif self.args.dataset == "NationalCSL-DP":
            pose_sample, _ = self.load_pose(sample['front_pose_path'])
            support_rgb_dict = None
        else:
        ...
        # action_info = generate_action_info(cated_pose_sample)

        return name_sample, pose_sample, text, gloss, support_rgb_dict, key, None
    
    def load_pose(self, path):
        if self.args.dataset == 'OpenASL':
            path = path.replace(":", "_")
        elif 'MSASL' in self.args.dataset:
            path = path.split("/")[-1]  
            # path = os.path.join(self.pose_dir, rel_path)
        elif 'WLASL' in self.args.dataset:   
            path = path.split("/")[-1].split('-')[0] 
            if not path.endswith('.pkl') and not path.endswith('.mp4'):
                path = path + '.pkl'
