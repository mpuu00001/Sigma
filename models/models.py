...
class Uni_Sign(nn.Module):
    ...
    def forward(self, src_input, tgt_input):
        ...
        output_hidden_states = True  if self.args.get_features else False
        ...
        if self.args.get_features:
            txt_features = self.get_txt_embedding(labels)
            stack_out = {
                'txt_features' : txt_features,
                'vis_features' : out['encoder_last_hidden_state'],
                'labels_txt' : tgt_input['gt_sentence']
                # 'label_keys':  tgt_input['label_keys']
            }
    ...
    def get_txt_embedding(self, labels):
        txt_input_ids = self.mt5_model._shift_right(labels).to(self.args.device)
        txt_embeddisng = self.mt5_model.decoder.embed_tokens(txt_input_ids)
        return txt_embeddisng
